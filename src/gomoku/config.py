"""Constants and configuration for Gomoku game."""

from enum import IntEnum


# ============ Player Enum ============
class Player(IntEnum):
    NONE = 0  # 空位
    BLACK = 1  # 黑棋 (先手)
    WHITE = 2  # 白棋


# ============ Game State Enum ============
class GameState(IntEnum):
    MENU = 0  # 颜色选择菜单
    PLAYING = 1  # 对局进行中
    GAME_OVER = 2  # 对局结束


# ============ Board Parameters ============
BOARD_SIZE: int = 15  # 棋盘行列数
GRID_SIZE: int = 53  # 格子间距 (pixels)
MARGIN: int = 43  # 棋盘边距 (pixels)，为坐标标注预留空间
WINDOW_SIZE: int = MARGIN * 2 + GRID_SIZE * (BOARD_SIZE - 1)  # 窗口边长
FPS: int = 30  # 帧率

# ============ Color Definitions ============
BLACK_COLOR: tuple[int, int, int] = (0, 0, 0)
WHITE_COLOR: tuple[int, int, int] = (255, 255, 255)
BG_COLOR: tuple[int, int, int] = (222, 184, 135)  # 棋盘背景色 (木色)
LINE_COLOR: tuple[int, int, int] = (0, 0, 0)  # 棋盘线条颜色
RED: tuple[int, int, int] = (255, 0, 0)  # 提示文字颜色

# ============ AI Configuration ============
AI_SEARCH_DEPTH: int = 4  # Minimax 最大搜索深度上限
AI_SEARCH_TIME_LIMIT_S: float | None = None  # 可选限时；None 表示仅按最大深度搜索
AI_MAX_CANDIDATES: int = 20  # 每层最多搜索候选点数（move ordering 后截断，减少搜索空间）
AI_CANDIDATE_RANGE: int = 2  # 候选点邻域半径；越大越宽，越小越快但更容易漏点
AI_ROOT_RERANK_TOP_K: int = 8  # 候选重排工具函数使用的前缀大小
AI_VCF_ENABLED: bool = True  # 是否启用 VCF 战术证明器
AI_VCF_MAX_DEPTH: int = 10  # VCF 最大递归深度（以攻击方回合层数计）
AI_VCF_MAX_CANDIDATES: int = 16  # VCF 每层最多扩展的强制进攻候选数
AI_QUIESCENCE_MAX_PLY: int = 8  # 静稳搜索工具函数的最大额外 ply
AI_QUIESCENCE_MAX_CANDIDATES: int = 3  # 静稳搜索工具函数的候选上限
AI_THREAT_EARLY_RETURN_ENABLED: bool = True  # 威胁候选工具函数的提前返回开关
AI_INTERNAL_FORCING_ENABLED: bool = True  # forcing search 工具函数的启用开关
AI_MOVE_DELAY_MS: int = 10  # AI 落子前的延时 (ms), 便于观察
AI_TT_MAX_SIZE: int = 100_000  # 置换表最大条目数；超限后清空，避免长局无限增长
AI_EVAL_CACHE_MAX_SIZE: int = 100_000  # 评估缓存最大条目数；超限后清空
