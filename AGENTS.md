...
目标是在可复现、正面对战测试里，把 `gomoku-test` 做强，并击败同级目录下的 `zhou`：
...
- 优先修复根因，不做只掩盖失败现象的补丁。
- `zhou` 基线固定为 `depth=5`。
- `gomoku-test` 正式对抗深度只能是 `4` 或 `5`，上限 `5`。
...
1. 确认现象还能复现。
2. 看 fixed opening matrix。
3. 拆白棋关键失败线。
4. 对比同一开局下 `depth=4` 与 `depth=5`。
5. 看 `W2` / `W4` 的候选与评分。
6. 看第一次 `vcf_block` / `immediate_block` 出现在哪一手。
7. 最后再看速度、节点数、耗时。
