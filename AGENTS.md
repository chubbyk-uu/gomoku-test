...
- 对该第 8 手局面的 root 候选继续拆解后已确认，问题不是单点：
  - 原始 `depth=5 minimax` 自己的根分数最高是旧路 `(7,1)`，而对白 `zhou` 可赢的 `(4,2)` 只排第二
  - early root rerank 又进一步把 `(4,2)` 的 rerank 分压到更后，最终把根决策改成 `(4,4)`
  - 更具体地说，`(4,2)` 这条对白 `zhou` 可赢的线，在白方 rerank probe 里连黑方真实关键 reply 都没进 top-3；probe 只看到了 `local_hotness` 排前的浅层 reply，导致对手建模失真
  - 同时，对 `(4,2)` 这个局面，黑方原始 depth-5 根分第一其实也是 `(7,4)`，但黑方自己的 rerank 又会把它改成别的点
  - 因此“为什么没选到胜手”当前应分成两层看：
    - 原始 `minimax` 为什么仍更偏好旧路 `(7,1)`
    - root rerank 为什么会把可赢候选 `(4,2)` 进一步压低并改选 `(4,4)`
- 当前 5 开局快速基线（rerank 开启）：
  - 白棋：`2胜 3负`
  - 黑棋：`5胜 0负`
...
- 因此，当前最该查的问题不再是“root rerank 是否复用 killer”，而是：
  - 为什么第 8 手 `minimax` 没有选到已经被实验验证可赢 `zhou` 的边界适配候选
  - 为什么原始 `minimax` 仍高估旧路 `(7,1)`，同时低估可赢候选 `(4,2)`
  - 为什么 root rerank 的 reply top-k 会漏掉黑方真实关键 reply，并把 `(4,2)` 进一步压低
...
2. 对第 8 手局面，先拆原始 `minimax` 根分数，解释为什么 `(7,1)` 仍排在 `(4,2)` 前面
3. 再拆 early root rerank 的 reply/stabilizer probe，解释为什么 `(4,2)` 会被进一步压低
4. 优先验证 rerank 的对手 reply top-k 为什么漏掉真实关键 reply
